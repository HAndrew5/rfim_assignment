#ifndef INCLUDE_RFIM_MEDIAN_PARALLEL_RFI
#define INCLUDE_RFIM_MEDIAN_PARALLEL_RFI
...
namespace rfim {
	...
	*/
	...
	class MedianParallelRfi : public RfiStrategy<MedianParallelRfi<DataType>>
	{
		static_assert(
			std::is_same<DataType, float>::value ||
			std::is_same<DataType, uint8_t>::value ||
			std::is_same<DataType, uint16_t>::value,
			"MedianParallelRfi DataType must be float, uint8_t, or uint16_t"
			);

	public:
		using StrategyDataType = DataType;

		MedianParallelRfi(TimeFrequencyMetadata metadata, float threshold = 4.5) :
